...
#
Cody Stephenson# CYEN406# Assignment3 - AES and SHA - 256
...
must be 16 bytes
for AES
BLOCK_SIZE = 16# pad at the end, so all blocks including the last one are 128 bit long
PAD_WITH = '#'
...
exit()# if first argument isn 't enc or dec	
if not((sys.argv[1] == "enc") or(sys.argv[1] == "dec")):
    print("Improper usage: First argument should be 'enc' or 'dec'")
exit()# if length of key isn 't 16, 24, or 32 throw error message	
...
def aes_func(encOrDec, key, filename): #16 - byte key.It can be 16, 20, or 32 bytes long.#Create a 'cipher'
object using 'secret_key'
cipher = AES.new(key)# open the file
f = open(filename, 'rb')
...
if (encOrDec) == "enc"
of.write(hashed_key)# if decrypting, grab the key from the beginning of input file
else :
    key_check = f.read(len(hashed_key))
if not(key_check == hashed_key):
    print("Invalid key!")
exit()# read the file one block_size at a time
